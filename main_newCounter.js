...
function countEditablesInCode(codeElement) {
  if (!codeElement) return {};

  const rawCode = codeElement.textContent || "";
  const decoded = decodeHTML(rawCode);
  ...
  result.loop = doc.querySelectorAll("loop").length;
  result.block = doc.querySelectorAll("block").length;

  return result;
}

function renderEditableResult(title, data) {
  const wrapper = document.createElement("div");
  wrapper.className = "blockReview__result";

  const heading = document.createElement("div");
  heading.className = "blockReview__title";
  heading.innerText = title;
  wrapper.appendChild(heading);

  const list = document.createElement("div");
  list.className = "blockReview__list";

  Object.entries(data).forEach(([key, value]) => {
    const span = document.createElement("span");
    span.className = `editable-count editable-${key}`;
  ...
  pcModules.forEach(pcModule => {
    if (spModules.includes(pcModule)) {
      ...
      if (pcBlock) {
        ...
        pcClone = pcBlock.cloneNode(true);
      ...
      if (spBlock) {
        spClone = spBlock.cloneNode(true);
      ...
      // TÍNH SỐ LƯỢNG EDITABLE từ text của <code>
      const pcHtmlCodeEl = pcClone ? pcClone.querySelector(".block-cont .edit-cont code") : null;
      const spHtmlCodeEl = spClone ? spClone.querySelector(".block-cont .edit-cont code") : null;
      const pcCountEditable = countEditablesInCode(pcHtmlCodeEl);
      const spCountEditable = countEditablesInCode(spHtmlCodeEl);
