...
export function moduleAddTextAllOn() {
  ...
  elField.forEach((el) => {
    ...
    while (
      ...
      && (parent && !parent.matches('div[id^="-loop"][id$="textarea"]'))
    ) {
...
export async function getCssFile(arrayColor) {
  document.querySelectorAll('input[name^="color"][name$="code"]').forEach((item,index) => {
    const newValue = arrayColor[index]||" ";
...
export function toggleLoopOn() {
  ...
  loopEls.forEach(el => {
    ...
    const minItems = parseInt(el.getAttribute('min'), 10);
    const maxItems = parseInt(el.getAttribute('max'), 10);
    ...
    function removeItem() {
      const currentItems = Array.from(el.children);
      if (currentItems.length > minItems) {
        el.removeChild(currentItems[currentItems.length - 1]);
        updateButtons();
      }
    }

    addMore.addEventListener('click', addItem);
    subtractBtn.addEventListener('click', removeItem);
  ...
  loopTb.forEach(el => {
    ...
    // Cập nhật trạng thái các nút (bật/tắt)
    function updateButtons() {
      const currentRows = el.querySelectorAll('.loop-table__row');
      const currentCols = el.querySelectorAll('.loop-table__column');
      
      // Cập nhật nút thêm/dùng số lượng hàng
      if (currentRows.length >= maxRows) {
        addMoreRow.disabled = true;
      } else {
        addMoreRow.disabled = false;
      }

      if (currentRows.length <= minRows) {
        subtractBtnRow.disabled = true;
      } else {
        subtractBtnRow.disabled = false;
      }

      // Cập nhật nút thêm/dùng số lượng cột
      if (currentCols.length >= maxCols) {
        addMoreCol.disabled = true;
      } else {
        addMoreCol.disabled = false;
      }

      if (currentCols.length <= minCols) {
        subtractBtnCol.disabled = true;
      } else {
        subtractBtnCol.disabled = false;
      }
    }
    ...
    function addRow() {
      const currentRows = el.querySelectorAll('.loop-table__row');
      if (currentRows.length < maxRows) {
        const firstRow = currentRows[2]; // Lấy hàng đầu tiên làm mẫu
        const newRow = firstRow.cloneNode(true); // Sao chép hàng
        el.appendChild(newRow); // Thêm vào cuối bảng
    ...
    function removeRow() {
      const currentRows = el.querySelectorAll('.loop-table__row');
      if (currentRows.length > minRows) {
        el.removeChild(currentRows[currentRows.length - 1]); // Xóa hàng cuối cùng
        updateButtons(); // Cập nhật trạng thái các nút
      }
    }
    ...
    function addColumn() {
      ...
      if (currentRows.length > 0) {
        ...
        const newColumn = firstRow.children[0].cloneNode(true); // Lấy cột đầu tiên và sao chép
        currentRows.forEach(row => {
          row.appendChild(newColumn.cloneNode(true)); // Thêm vào tất cả các hàng
        });
        updateButtons(); // Cập nhật trạng thái các nút
      }
    }
      ...
      if (currentRows.length > 0) {
        currentRows.forEach(row => {
          if (row.children.length > minCols) {
            row.removeChild(row.lastElementChild); // Xóa cột cuối cùng trong mỗi hàng
          }
        });
        updateButtons(); // Cập nhật trạng thái các nút
      }
    ...
    updateButtons(); // Cập nhật trạng thái các nút khi khởi tạo
  });
